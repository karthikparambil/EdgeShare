...
(async () => {
  ...
  const res = await fetch('http://localhost:9999/api/qr');
  const data = await res.json();
  const token = data.token;
  
  const desktop = io('http://localhost:9999', { auth: { token, role: 'desktop' } });
  ...
  setTimeout(() => {
      const phone = io('http://localhost:9999', { auth: { token, role: 'phone' } });
